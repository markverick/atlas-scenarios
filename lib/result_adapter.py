...
def parse_conv_trace(conv_trace_path):
    ...
    For multi-metric app traces, this returns prefix_propagation_s.
    """
    metrics = parse_conv_trace_metrics(conv_trace_path)
    if metrics["prefix_propagation_s"] >= 0:
        return metrics["prefix_propagation_s"]
    if metrics["router_reachability_s"] >= 0:
        return metrics["router_reachability_s"]
    return -1


def _parse_conv_value(raw_value):
    try:
        t = float(raw_value)
...
def parse_conv_trace_metrics(conv_trace_path):
    """Read named convergence metrics written by the C++ scenario.

    New app traces store one metric per line as key=value. Legacy one-line
    traces are interpreted as prefix_propagation_s for backward compatibility.
    """
    metrics = {
    ...
    if len(lines) == 1 and "=" not in lines[0]:
        metrics["prefix_propagation_s"] = _parse_conv_value(lines[0])
        return metrics
    ...
    for line in lines:
        ...
        if key not in metrics:
            continue
        metrics[key] = _parse_conv_value(value.strip())
...
def sim_trial_result(grid_size, num_nodes, num_links, rate_trace_path,
    ...
    """
    metrics = parse_conv_trace_metrics(conv_trace_path)
    conv = metrics["prefix_propagation_s"]
    ...
    return TrialResult(
        ...
        router_reachability_s=metrics["router_reachability_s"],
        transfer_ok=conv >= 0,
        avg_mem_kb=0,
        **traffic,
    )
